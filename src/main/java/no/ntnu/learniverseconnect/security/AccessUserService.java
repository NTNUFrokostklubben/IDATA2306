...
public class AccessUserService implements UserDetailsService {
  ...
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    ...
    System.out.println("User found: " + user.get().getName()+" + is active: "+user.isPresent());
    if (user.isPresent()) {
  ...
  public void tryCreateNewUser(String username, String password){
    String errormessage;
    if(username.isEmpty()){
  ...
  public String checkPasswordRequirements(String password){
    ...
    if (password == null || password.length()==0 || password.isEmpty()){
